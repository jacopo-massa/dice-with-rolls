...
def _get_all_stories():
    ...
    return render_template("stories.html", stories=stories, home_url=HOME_URL)
...
def _get_range():
    ...
    if check_service_up(x):
        ...
        if x.status_code < 300:
            return render_template("stories.html", stories=body, home_url=HOME_URL)
...
def _get_drafts():
    ...
    return render_template("drafts.html", drafts=stories, home_url=HOME_URL)
...
def _get_write_page():
    ...
    return render_template("write_story.html", form=form, id_draft=None, words=session['figures'], home_url=HOME_URL)
    ...
    if check_service_up(x):
        ...
        # Else reload the page with the specified error
            ...
            return render_template("write_story.html", message=body['description'], id_draft=None,
                                   form=new_form, words=session['figures'], home_url=HOME_URL)
    else:
        return redirect(url_for('gateway._home'))
...
def render_story(story=None):
    context_vars = {"home_url": HOME_URL, "react_url": HOME_URL + 'stories/{}/react',
